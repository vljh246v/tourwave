...
- Features module: ❌
- API client usage: ❌
- SSOT reference: openapi.yaml tag `Reports`

## BE OpenAPI Paths (4 endpoints)

- `get /organizations/{orgId}/reports/bookings/export` — Booking report (ORG_ADMIN/ORG_OWNER)
- `get /organizations/{orgId}/reports/bookings` — Export occurrence ops report CSV (ORG_ADMIN/ORG_OWNER)
- `get /organizations/{orgId}/reports/occurrences/export` — Occurrence ops report (ORG_ADMIN/ORG_OWNER)
- `get /organizations/{orgId}/reports/occurrences` — Organization review summary (operator)
