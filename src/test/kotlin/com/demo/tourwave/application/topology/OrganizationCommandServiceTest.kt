...
class OrganizationCommandServiceTest {
    ...
    fun `create organization creates owner membership and public profile`() {
        ...
        val created = organizationCommandService.createOrganization(
    ...
    fun `membership lifecycle supports invite accept role change and deactivate`() {
        ...
        val invited = membershipService.invite(
        ...
        val accepted = membershipService.acceptInvitation(
        ...
        val promoted = membershipService.changeRole(
        ...
        val deactivated = membershipService.deactivate(
    ...
    fun `admin cannot assign owner role or deactivate self`() {
        val owner = userRepository.save(User.create(displayName = "Owner", email = "owner@test.com", passwordHash = "hash", now = clock.instant()))
        val admin = userRepository.save(User.create(displayName = "Admin", email = "admin@test.com", passwordHash = "hash", now = clock.instant()))
        val organization = organizationCommandService.createOrganization(
            CreateOrganizationCommand(
                actorUserId = requireNotNull(owner.id),
                slug = "jeju-trails",
