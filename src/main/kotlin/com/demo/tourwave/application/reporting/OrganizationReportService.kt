...
class OrganizationReportService(
    ...
    fun getBookingReport(query: BookingReportQuery): CursorPage<BookingReportItemView> {
        ...
        val tourIds = query.tourId?.let { setOf(it) } ?: tourRepository.findByOrganizationId(query.organizationId).mapNotNull { it.id }.toSet()
        ...
        val filtered = bookingRepository.findAll()
            .filter { it.organizationId == query.organizationId }
            ...
            .sortedWith(compareByDescending<com.demo.tourwave.domain.booking.Booking> { it.createdAt }.thenByDescending { it.id ?: -1L })

        val paged = filtered.pageAfter(query.cursor, safeLimit) { requireNotNull(it.id) }
        return CursorPage(
            items = paged.items.map { booking ->
    ...
    fun getOccurrenceOpsReport(query: OccurrenceOpsReportQuery): CursorPage<OccurrenceOpsReportItemView> {
        ...
        val filtered = occurrenceRepository.findAll()
            .filter { it.organizationId == query.organizationId }
            .filter { query.occurrenceId == null || it.id == query.occurrenceId }
            .filter { it.tourId == null || it.tourId in tourIds }
            .filter { occurrence ->
            ...
            .sortedWith(compareByDescending<com.demo.tourwave.domain.occurrence.Occurrence> { it.startsAtUtc ?: Instant.EPOCH }.thenByDescending { it.id })

        val paged = filtered.pageAfter(query.cursor, safeLimit) { it.id }
        return CursorPage(
            items = paged.items.map { occurrence ->
                ...
                val confirmedSeats = bookings.filter { it.status == BookingStatus.CONFIRMED || it.status == BookingStatus.COMPLETED }
                    .sumOf { it.partySize }
                val waitlistCount = bookings.count { it.status == BookingStatus.WAITLISTED }
                val attendedCount = participants.count {
                ...
                val noShowCount = participants.count {
                    (it.status == BookingParticipantStatus.LEADER || it.status == BookingParticipantStatus.ACCEPTED) &&
                ...
                val refundPendingCount = paymentRecords.count {
                ...
                OccurrenceOpsReportItemView(
                    ...
                    seatUtilizationPercent = if (occurrence.capacity <= 0) 0 else ((confirmedSeats.toDouble() / occurrence.capacity.toDouble()) * 100).roundToInt(),
    ...
    private fun <T> List<T>.pageAfter(cursor: String?, limit: Int, idSelector: (T) -> Long): PageSlice<T> {
        val filtered = cursor?.toLongOrNull()?.let { cursorId ->
            dropWhile { idSelector(it) != cursorId }.drop(1)
        } ?: this
        val items = filtered.take(limit)
        val nextCursor = items.takeIf { filtered.size > limit }?.lastOrNull()?.let(idSelector)?.toString()
