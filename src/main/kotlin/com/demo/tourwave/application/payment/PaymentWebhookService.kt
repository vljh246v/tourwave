...
class PaymentWebhookService(
    ...
    private val resolvedSecrets = webhookSecrets
    ...
    fun recordMalformedPayload(rawPayload: String, signature: String?, reason: String? = null) {
        val now = clock.instant()
        ...
        paymentProviderEventRepository.save(
            PaymentProviderEvent(
                ...
                status = if (signatureVerification.valid) {
    ...
    fun expectedSignature(rawPayload: String, keyId: String? = null): String {
    ...
    private fun verifySignature(rawPayload: String, signature: String?): SignatureVerification {
    ...
    private fun sign(secret: String, rawPayload: String): String {
    ...
    private fun parseSignatureHeader(rawSignature: String): ParsedSignature {
        return when {
            rawSignature.contains(',') -> {
                val pairs = rawSignature.split(',').mapNotNull { token ->
            ...
            else -> ParsedSignature(keyId = null, signature = rawSignature)
        }
    }

    private fun sha256(rawPayload: String): String {
        val digest = MessageDigest.getInstance("SHA-256")
        return digest.digest(rawPayload.toByteArray(StandardCharsets.UTF_8))
