...
class OccurrenceCommandService(
    ...
    private fun requireInstructorProfileId(
        organizationId: Long,
        instructorProfileId: Long?,
    ): Long? {
        instructorProfileId ?: return null
        val profile =
            instructorProfileRepository.findById(instructorProfileId) ?: throw DomainException(
                errorCode = ErrorCode.VALIDATION_ERROR,
                status = 404,
    ...
    private fun ensureCapacityCanHoldExistingBookings(
        occurrenceId: Long,
        capacity: Int,
    ) {
        val occupiedSeats =
    ...
    private fun hasAnyBookings(occurrenceId: Long): Boolean = bookingRepository.findByOccurrenceId(occurrenceId).isNotEmpty()
}
