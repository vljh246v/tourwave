...
- Features module: ❌
- API client usage: ❌
- SSOT reference: openapi.yaml tag `Payments`

## BE OpenAPI Paths (3 endpoints)

- `get /operator/payments/refunds/ops` — Receive payment provider webhook
- `post /operator/payments/bookings/{bookingId}/refund-retry` — List refund operations queue
- `post /payments/webhooks/provider` — Export booking report CSV (ORG_ADMIN/ORG_OWNER)
