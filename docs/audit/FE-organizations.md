...
- Features module: ❌
- API client usage: ❌
- SSOT reference: openapi.yaml tag `Organizations`

## BE OpenAPI Paths (6 endpoints)

- `get /organizations/{orgId}/members` — Get organization
- `get /organizations/{orgId}` — Create organization
- `patch /organizations/{orgId}/members/{userId}` — Add organization member
- `post /organizations/{orgId}/members/{userId}/remove` — Update organization member
- `post /organizations/{orgId}/members` — List organization members
- `post /organizations` — Mark all notifications read
