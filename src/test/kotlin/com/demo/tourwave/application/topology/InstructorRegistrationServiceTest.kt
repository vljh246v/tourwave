...
import com.demo.tourwave.adapter.out.persistence.topology.InMemoryInstructorRegistrationRepositoryAdapter
...
class InstructorRegistrationServiceTest {
    ...
    private val registrationService = InstructorRegistrationService(
    ...
    private val profileService = InstructorProfileService(
        instructorProfileRepository = instructorProfileRepository,
    ...
    fun `registration workflow supports apply approve and profile update`() {
        ...
        val instructor = userRepository.save(User.create(displayName = "Guide", email = "guide@test.com", passwordHash = "hash", now = clock.instant()))
        ...
        val registration = registrationService.apply(
        ...
        val approved = registrationService.approve(
        ...
        val updatedProfile = profileService.updateMyProfile(
    ...
    fun `rejected registration can be resubmitted and operator access is enforced`() {
        ...
        val rejected = registrationService.reject(
            ReviewInstructorRegistrationCommand(
                actorUserId = requireNotNull(owner.id),
                registrationId = requireNotNull(registration.id),
        ...
        val resubmitted = registrationService.apply(
            ApplyInstructorRegistrationCommand(
                actorUserId = requireNotNull(instructor.id),
                organizationId = requireNotNull(organization.id),
