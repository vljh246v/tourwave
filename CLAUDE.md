...
| 경로 | 용도 |
|---|---|
| `docs/tasks/M1/` | M1 마일스톤 태스크 카드 (T-001~T-007 등). 카드에는 ID/제목/AC/검증 명령 포함 |
| `docs/tasks/cross/` | 마일스톤 무관 cross-cutting 태스크 (린트, 감사 커버리지, 인덱스 동기화 등) |
| `docs/exec-plans/active/` | 진행 중 실행 플랜. 한 태스크의 단계별 체크리스트와 결정 기록 |
| `docs/exec-plans/completed/` | 완료된 실행 플랜 아카이브. 회고 시 참조 |
| `docs/exec-plans/tech-debt-tracker.md` | 즉시 수정 보류한 기술 부채 누적 추적 (TD-001 등) |
| `docs/session-notes/` | 일자별 세션 핸드오프 노트 (`YYYY-MM-DD.md`) |
| `docs/audit/` | 도메인별 감사 보고서. `BE-<도메인>.md`(백엔드), `FE-<도메인>.md`(프론트), `_index.md`/`FE-SUMMARY.md` |
| `docs/gap-matrix.md` | 스펙 vs 구현 갭 매트릭스 (도메인별 미구현/부분구현 추적) |
| `logs/trends/failure-patterns.md` | 최근 검증 실패 패턴 누적 로그 (Feedforward용) |
...
- 감사 커버리지: T-904 (2026-04-26) 완료. announcement/organization/instructor/tour 도메인은 application 단위 테스트(`*AuditTest.kt` 5종)로 검증. 예약·결제 영역은 여전히 통합 테스트에서 부분 검증 — 추가 단위 테스트 보강 후보
