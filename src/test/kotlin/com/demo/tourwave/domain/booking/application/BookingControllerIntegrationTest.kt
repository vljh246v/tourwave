...
class BookingControllerIntegrationTest {
    ...
    fun `create booking returns REQUESTED when available seats are enough`() {
        ...
        assertEquals(101L, participants.single().userId)
        assertEquals("LEADER", participants.single().status.name)
    }
    ...
    fun `same idempotency key and payload returns identical booking`() {
        ...
        mockMvc
            ...
            .andExpect { result ->
                assertEquals(first, result.response.contentAsString)
            }
    }
    ...
    fun `approve booking returns 204 and updates booking status`() {
        ...
        assertEquals(BookingStatus.CONFIRMED, updated?.status)
    ...
    fun `reject booking returns 204 and updates to REJECTED`() {
        ...
        val updated = bookingRepository.findById(requireNotNull(saved.id))
        assertEquals(BookingStatus.REJECTED, updated?.status)
        assertEquals(PaymentStatus.REFUNDED, updated?.paymentStatus)
    }
    ...
    fun `cancel booking returns 204 and refunds payment`() {
        ...
        val updated = bookingRepository.findById(requireNotNull(saved.id))
        assertEquals(BookingStatus.CANCELED, updated?.status)
        assertEquals(PaymentStatus.REFUNDED, updated?.paymentStatus)
        assertEquals(PaymentRecordStatus.REFUNDED, paymentRecordRepository.findByBookingId(requireNotNull(saved.id))?.status)
    }
    ...
    fun `cancel booking leaves refund pending when refund execution is retryable failure`() {
        ...
        val updated = bookingRepository.findById(requireNotNull(saved.id))
        assertEquals(BookingStatus.CANCELED, updated?.status)
        assertEquals(PaymentStatus.REFUND_PENDING, updated?.paymentStatus)
        assertEquals(PaymentRecordStatus.REFUND_FAILED_RETRYABLE, paymentRecordRepository.findByBookingId(requireNotNull(saved.id))?.status)
    }
    ...
    fun `decline offer returns 204 and updates to EXPIRED`() {
        ...
        val updated = bookingRepository.findById(requireNotNull(saved.id))
        assertEquals(BookingStatus.EXPIRED, updated?.status)
        assertEquals(PaymentStatus.REFUNDED, updated?.paymentStatus)
    }
    ...
    fun `occurrence cancel cascades non-terminal bookings and blocks offer flow`() {
        ...
        assertEquals(BookingStatus.CANCELED, bookingRepository.findById(requireNotNull(requested.id))?.status)
        assertEquals(BookingStatus.CANCELED, bookingRepository.findById(requireNotNull(offered.id))?.status)
        assertEquals(BookingStatus.EXPIRED, bookingRepository.findById(requireNotNull(expired.id))?.status)
    ...
    fun `party size patch idempotency replay and in-progress follow policy`() {
        ...
        mockMvc
            ...
            .andExpect { result -> assertEquals(firstBody, result.response.contentAsString) }
    ...
    fun `booking complete returns 204 and transitions to COMPLETED with idempotent replay`() {
        ...
        assertEquals(BookingStatus.COMPLETED, bookingRepository.findById(requireNotNull(booking.id))?.status)
    }
    ...
    fun `occurrence finish returns 204 then blocks new booking with 409 code`() {
        ...
        assertEquals(OccurrenceStatus.FINISHED, occurrenceRepository.getOrCreate(73033L).status)
    ...
    fun `inquiry create is idempotent and writes audit minimum contract`() {
        ...
        assertEquals("INQUIRY_CREATED", lastEvent.action)
        assertEquals("INQUIRY", lastEvent.resourceType)
        assertEquals("req-inq-001", lastEvent.requestId)
    ...
    fun `inquiry messages allow leader and org operator`() {
        ...
        assertEquals("INQUIRY_MESSAGE_POSTED", operatorEvent.action)
        assertEquals("INQUIRY_MESSAGE", operatorEvent.resourceType)
        assertEquals("req-msg-002", operatorEvent.requestId)
        assertEquals("OPERATOR:990", operatorEvent.actor)
    }
    ...
    fun `operator can manually promote and skip waitlist bookings`() {
        ...
        assertEquals("WAITLIST_SKIPPED_MANUALLY", actions[0].action)
        assertEquals("큰 그룹이라 뒤로", actions[0].details["note"])
        assertEquals("WAITLIST_PROMOTED_MANUALLY", actions[1].action)
        assertEquals("빈자리 발생", actions[1].details["note"])
    }
    ...
    fun `inquiry close enforces state transition and idempotency with audit`() {
        ...
        assertEquals(InquiryStatus.CLOSED, inquiryRepository.findById(requireNotNull(inquiry.id))?.status)
        val lastEvent = auditEventAdapter.all().last()
        assertEquals("INQUIRY_CLOSED", lastEvent.action)
        assertEquals("INQUIRY", lastEvent.resourceType)
        assertEquals("req-close-001", lastEvent.requestId)
        assertEquals("USER:884", lastEvent.actor)
        assertEquals(
            1,
    ...
    fun `cancel confirmed booking promotes waitlist and creates offer window`() {
        ...
        assertEquals(BookingStatus.OFFERED, promoted?.status)
