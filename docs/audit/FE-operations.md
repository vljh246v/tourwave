...
- Features module: ❌
- API client usage: ❌
- SSOT reference: openapi.yaml tag `Operations`

## BE OpenAPI Paths (2 endpoints)

- `get /operator/operations/remediation-queue` — Export reconciliation summaries as CSV
- `post /operator/operations/remediation-queue/{sourceType}/{sourceKey}` — List cross-surface operator remediation queue
