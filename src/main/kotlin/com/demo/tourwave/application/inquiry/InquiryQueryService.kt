...
class InquiryQueryService(
    ...
    fun listMyInquiries(query: ListMyInquiriesQuery): InquiryListResult {
        ...
        val inquiries = inquiryRepository.findByCreatedByUserId(query.actorUserId)
            .asSequence()
    ...
    private fun com.demo.tourwave.domain.inquiry.Inquiry.toDetailView(): InquiryDetailView {
        return InquiryDetailView(
            ...
            status = status,
            createdAt = createdAt,
            lastMessageAt = inquiryRepository.findMessagesByInquiryId(requireNotNull(id)).maxOfOrNull { it.createdAt }
        )
    }
    ...
    private fun parseCursor(cursor: String?, fieldName: String): Long? {
