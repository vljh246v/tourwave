...
class OrganizationMembershipService(
    ...
    fun invite(command: InviteOrganizationMemberCommand): OrganizationMembership {
        ...
        val targetUser = userRepository.findById(command.userId) ?: throw DomainException(
        ...
        val saved = when {
            existing == null -> OrganizationMembership.invite(
    ...
    fun changeRole(command: ChangeOrganizationMemberRoleCommand): OrganizationMembership {
        val actorMembership = organizationAccessGuard.requireOperator(command.actorUserId, command.organizationId)
        val membership = membershipRepository.findByOrganizationIdAndUserId(command.organizationId, command.memberUserId)
            ?: throw DomainException(
                errorCode = ErrorCode.VALIDATION_ERROR,
                status = 404,
    ...
    fun listMemberships(actorUserId: Long, organizationId: Long): List<OrganizationMembership> {
